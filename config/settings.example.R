...
make_volcano_plots <- TRUE                    # options: TRUE/FALSE (if FALSE, volcano plots will not be generated, but the data for them will still be exported and can be plotted later)
...
heatmap_top_n <- 80                           # options: integer > 0 (number of top features to show in the heatmap, based on the metric specified in heatmap_rank_metrics)
...
# Output control
minimal_outputs <- FALSE                        #  options: TRUE/FALSE. If TRUE, only exports a minimal set of results (mostly tables, no plots). 
